...
void RustController::init(const Identity& signingId, Sender* sender)
{
    ...
    if (!rust_controller_init(addr, _homePath.c_str())) {
        fprintf(stderr, "[RustController] WARNING: failed to initialize Rust controller\n");
    }
}
    ...
    const Dictionary<ZT_NETWORKCONFIG_METADATA_DICT_CAPACITY>& metaData)
{
    ...
    struct sockaddr_in6* routeTarget = reinterpret_cast<struct sockaddr_in6*>(&nc.routes[0].target);
    ...
    nc.com = CertificateOfMembership(nc.timestamp, CREDENTIAL_TIME_MAX_DELTA, nwid, identity);
